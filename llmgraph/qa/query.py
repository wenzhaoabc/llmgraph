...
def search_extities_by_keywords(keywords: list[str]) -> list["Entity"]:
    ...
    for e in entities:
        ...
        if any([w.upper() in e_words for w in keywords]):
            ...
            [w.upper() in e_words for w in e.properties.get("_alias")]
        ):
            result.append(e)
        elif e.properties.get("acronym") and any(
            [w.upper() in e_words for w in e.properties.get("acronym")]
        ):
            result.append(e)
    return result
